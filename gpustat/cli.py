...
def print_gpustat(
    *,
    ...
    json=False,
    debug=False,
    show_cuda_device_order_fallback_warning=True,
    **kwargs,
):
    ...
    if (
        show_cuda_device_order_fallback_warning
        and gpu_stats.cuda_device_order_fallback_reason
    ):
        sys.stderr.write(
            "Warning: %s; using default NVML GPU order.\n"
            % gpu_stats.cuda_device_order_fallback_reason
        )
        sys.stderr.flush()
    ...
    with term.fullscreen():
        while 1:
            try:
                ...
                show_cuda_device_order_fallback_warning = False
...
def main(*argv):
    ...
    parser.add_argument(
        "-a", "--show-all", action="store_true", help="Display all gpu properties above"
    )
    parser.add_argument(
        "-c",
        "--show-cmd",
    ...
    parser.add_argument(
        "-p", "--show-pid", action="store_true", help="Display PID of running process"
    )
    parser.add_argument(
        "-F",
        "--show-fan-speed",
    ...
    if args.interval > 0:
        ...
        if args.json:
            sys.stderr.write(
                "Error: --json and --interval/-i " "can't be used together.\n"
            )
            sys.exit(1)
